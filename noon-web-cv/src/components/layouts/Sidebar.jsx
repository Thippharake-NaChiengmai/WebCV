...
export default function Sidebar() {
  ...
  return <aside className={`col-12 col-md-3 bg-success text-white p-3 d-flex flex-column position-relative overflow-hidden sidebar ${open ? 'show-full' : ''}`}>
    ...
    <div className="d-md-none text-center mt-3"><button onClick={() => setOpen(!open)} className="btn btn-outline-light w-100">{open ? 'Show Less' : 'Show More'}</button></div>
  </aside>;
}
