...
const Details = () => {
  ...
  useEffect(() => {
    ...
    PortfolioAPI.getProjectById(Number(projectId)).then(response => {
      ...
      setProject(response.data); setActiveImage(0);
      return PortfolioAPI.getRelatedProjects(Number(projectId));
    }).then(response => setRelatedProjects(response.data)).catch(error => setError(error.message));
  }, [projectId]);
  ...
  const hasActions = project.githubUrl || project.youtubeUrl || project.websiteUrl || project.figmaUrl !== undefined || project.download;
