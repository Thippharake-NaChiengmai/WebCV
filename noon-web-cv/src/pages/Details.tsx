...
const Details = () => {
  ...
  useEffect(() => {
    if (!projectId) return;
    PortfolioAPI.getProjectById(Number(projectId))
      .then(response => {
      ...
      .then(response => setRelatedProjects(response.data))
  ...
  const hasActions = project.githubUrl || project.youtubeUrl || project.figmaUrl !== undefined || project.download;
